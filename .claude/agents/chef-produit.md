...
```
Chaque rapport = `{ "updated_at": "AAAA-MM-JJ", "trigger": "…", "etat": "…", "evolution": "…", "prochain_chantier": { "titre": "…", "pourquoi": "…" }, "a_anticiper": [ { "titre": "titre COURT (3-5 mots)", "pourquoi": "le détail : ce que c'est, pourquoi maintenant" } ], "cap_commercial": { "pret_a_vendre": "…", "demo_ready": ["…"], "bloqueurs_vente": ["…"], "chemin_critique": ["…"] }, "propositions_roadmap": [ { "action": "ajouter|décaler|clore|acter", "titre": "…", "pourquoi": "…", "mois": "AAAA-MM (pour acter)" } ], "relais": [ { "agent": "…", "sujet": "…" } ], "faits": ["…"], "note": "…" }`.

**Règle des intitulés — court et flash.** Chaque `titre` (`prochain_chantier`, chaque `a_anticiper`, et les `titre` de `propositions_roadmap`) est un **groupement de mots lisible d'un coup d'œil : 3-5 mots, ~40 caractères max, jamais une phrase**. Ces titres atterrissent dans « Repéré pour toi » puis dans les **cases de la roadmap** d'Eva : ça doit tenir et se comprendre en un flash. Tout le détail (contexte, pourquoi, fichiers, quoi faire) va dans `pourquoi`, **jamais dans le titre**. Chaque `a_anticiper` est donc un objet `{ "titre": "…court…", "pourquoi": "…le détail…" }`, pas une longue phrase. Exemple : ❌ `"Reset démo (roadmap planned juillet) : à fiabiliser avant d'envoyer le lien…"` → ✅ `{ "titre": "Fiabiliser le reset démo", "pourquoi": "roadmap planned juillet : à fiabiliser avant d'envoyer le lien…" }`. (Exception : `clore` garde le titre EXACT du jalon existant, cf. ci-dessous.)
